          ...
          (let [path (conj path (first xs))
            ...
            (recur xs
                   ...
                   (conj res (merge {:text (or (path-names x) (str x))}
                                    (when (seq xs) {:actions [[:set-path (first path) (rest path)]]})))))
...
(defmethod prep-val :string [label path s]
  (cond
    ...
    {:val (pr-str (str (first (str/split s #"\.")) "..."))
     :type :jwt
     :actions [[:set-path label (conj (vec path) :gadget/JWT)]]}
    ...
    {:path (prepare-path (concat [label] path))
     :data (when v
             (let [[ks vs] (key-vals v)]
               (map vector (map prep-key ks) (map #(prep-top-level-val label (conj path %1) %2) ks vs))))
