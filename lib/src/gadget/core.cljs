    ...
    (let [ks (keys m)]
      (assoc
       (if (too-long-for-inline? ks)
         ...
         {:type :map-keys :val (map prep-val (sort-by str ks))})
       :actions [[:set-path "???" []]]))
...
(defn- prep-top-level-val [v]
  (assoc (prep-val v) :copyable (pr-str v)))
    ...
    {:path (prepare-path (concat [label] path))
     :data (when v
             (let [[ks vs] (key-vals v)]
               (->> (map vector (map prep-key ks) (map prep-top-level-val vs))
                    (into {}))))
