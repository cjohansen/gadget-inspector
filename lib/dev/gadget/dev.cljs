(ns ^:figwheel-hooks gadget.dev
  (:require [cljs-data-browser.actions :as actions]
            ...
            [gadget.core :as g]
            [gadget.datafy :as d]
            [gadget.datascript :as datascript]
            [gadget.dev-renderer]
            [gadget.extensions]
            [datascript.db :as db]))

(def invoices
  (list {:dueamount 182.31, :kid "755120058905", :due-date "2018-11-27T00:00:00", :invoice-no "518009794", :code "", :account-no "", :name ""}
...
(defonce conn (let [conn (ds/create-conn {:user/id {:db/unique :db.unique/identity}
                                          :user/friends {:db/type :db.type/ref
                                                         :db/cardinality :db.cardinality/many}})]
                #_(ds/transact! conn [{:user/id (random-uuid) :user/name "Kari Ku"}])
                (ds/transact! conn [{:user/id #uuid "ea75f3b6-3f91-486b-900d-5e7fcae9fd55" :user/name "Christian Johansen"}])
                (ds/transact! conn [{:user/id (random-uuid)
                                     :user/name "New Guy"
                                     :user/friends [{:user/id #uuid "ea75f3b6-3f91-486b-900d-5e7fcae9fd55"}]}])
                #_(ds/transact! conn [{:user/id (random-uuid) :user/name "Arne And"}])
                conn))

(g/inspect "Datascript connection" conn)
...
(comment
  ...
  (require '[datascript.core :as ddd])

  (ds/transact! conn [{:user/id #uuid "4739a8c3-241a-4d47-9f9e-bc078cfdabd0"
                       :user/friends [{:user/id (random-uuid) :user/name "Arne And"}]}])
  ...
  (type (ddd/entity (ddd/db conn) (first (sort (set (map first (:eavt @conn)))))))
  ...
  (type (d/nav-in @conn [:aevt]))

  (d/symbolic-type (:aevt @conn))

  (d/get-symbolic-type (first (:aevt @conn)))

  (type (first (:aevt @conn)))

  (.-name (.-constructor js/window.location))

  (re-find #"#([^\s]+)" (with-out-str (cljs.pprint/pprint (first (:aevt @conn)))))
  (with-out-str (cljs.pprint/pprint js/window))
...
(def data
  {:apiKey "POK3uZiVx+aiWcwHKQXZ9N0ENcV9aWnoEmsLMnQhYezssdfhj78"
   ...
   :invoices invoices
   :invoice-seq (map identity invoices)
   :invoice-set (into #{} invoices)
...
(defonce store (g/create-atom "App state" data))

(comment
  (swap! store assoc :now (js/Date.))
  (swap! store with-meta {:lol 42})

  (swap! store assoc :lol (with-meta {:ok? true} {:haha 42}))

  (swap! store merge {:now (js/Date.)
                      :gadget.tx/data {:action [:do-something 42]}})

  (defn Person [name]
    (this-as this
      (set! (.-name this) name)))

  (swap! store merge {:person (Person. "Bob")
                      :gadget.tx/data {:action [:add-person "Bob"]}})

  (swap! store assoc :invoice-amounts (map #(select-keys % [:dueamount]) (:invoices @store)))

  (type (:invoice-amounts @store))

  (d/nav-in @store [:invoice-amounts 0])

  (type (clojure.datafy/datafy (:invoice-amounts @store)))

  (gadget.browsable/entries [:a :b :c :d :e {:ok true}])

  (first (:invoice-amounts @store))
